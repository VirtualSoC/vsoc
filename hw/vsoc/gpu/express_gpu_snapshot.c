...
//     LOGD("saving vertex array num %d", g_hash_table_size(resource_list));
...
//         LOGD("saving vertex array ID: %d", vertex_array);
...
//         LOGD("loading vertex array ID: old %d new %d",vertex_array_id, new_vertex_array_id);
...
void save_native_buffer(QEMUFile *f, GLint buffer_id, GLenum buffer_target) {
    LOGD("saving buffer ID: %d, Type: %d", buffer_id, buffer_target);
    ...
    if (size > 0) {
        ...
        if (data) {
            ...
            LOGD("saving buffer of content length %d", size);
...
void load_native_buffer(QEMUFile *f, GLint buffer_id, GLenum buffer_target, int strategy) {
    ...
    LOGD("in load native buffer change host id maps %d %d", buffer_id, new_buffer_id);
    ...
    if (size > 0) {
        ...
        if (data) {
            ...
            LOGD("loading buffer context of size 0");
            free(data);
        }
    }
}
...
void save_native_buffers(QEMUFile *f) {
    ...
    LOGD("saving buffer num %d", g_hash_table_size(resource_list));
    ...
    while (g_hash_table_iter_next(&iter, &key, &value)) {
        ...
        LOGD("saving buffer ID: %d, Type: %d", buffer->bufferId, buffer->target);
        ...
        if (size > 0) {
            ...
            if (data) {
                ...
                if (error != GL_NO_ERROR) {
                    LOGD("error when reading buffer data");
                }
...
void load_native_buffers_tmp(QEMUFile *f) {
    ...
    for(int i = 0; i < buffer_num ; i++) {
        ...
        LOGD("loading buffer of id %d size %d type %x", buffer_id, size, target);
        ...
        if (size > 0) {
            ...
            if (data) {
                ...
                // if(size > 40000)
                //     LOGD("before loading buffer context first 4 bytes %x %x %x %x", ((char*)data)[9140], ((char*)data)[141], ((char*)data)[42], ((char*)data)[49151]);
                ...
                // glGetBufferSubData(target, 0, size, data);
                // if(size > 40000)
                //     LOGD("loading buffer context first 4 bytes %x %x %x %x", ((char*)data)[1], ((char*)data)[12], ((char*)data)[22], ((char*)data)[16]);
                // GLuint glerror = glGetError();
                // if (glerror != GL_NO_ERROR) {
                //     LOGD("error when loading buffer!");
                // }
                g_free(data);
            }
        } else {
        ...
        LOGD("current buffer size is %d", newsize);
...
void load_native_buffers(QEMUFile *f) {
    ...
    for(int i = 0; i < buffer_num ; i++) {
        ...
        // if(size == 49152) {
        //     LOGD("loading buffer ID: %d, Type: %x, Size: %d", buffer_id, target, size);
        ...
        LOGD("loading buffer ID: old %d new %d, Type: %x, Size: %d strategy %d", buffer_id, new_buffer_id, target, size, strategy);
        ...
        if (size > 0) {
            ...
            if (data) {
                ...
                // LOGD("real data loading buffer context first 4 bytes %x %x %x %x", ((char*)data)[1], ((char*)data)[12], ((char*)data)[22], ((char*)data)[16]);
...
void save_single_framebuffer(QEMUFile* f, Express_Native_Framebuffer *framebuffer){
    ...
    for(int i = 0; i < 16; i++) {
        ...
        LOGD("saving framebuffer attachment %d %d", i, framebuffer->attachment_target[i]);
    }
    for(int i = 0; i < 19; i++) {
        qemu_put_be32(f, framebuffer->renderbuffer_attachment[i]);
        LOGD("saving framebuffer renderbuffer attachment %d %d", i, framebuffer->renderbuffer_attachment[i]);
    }
    for(int i = 0; i < 19; i++) {
        qemu_put_be32(f, framebuffer->renderbuffer_attachment[i]);
    ...
    //     LOGD("saving framebuffer attachment %d %d", i, attachment_object);
    ...
    // LOGD("saving framebuffer all info %d %d %d", framebufferId, depth_attachment, stencil_attachment);
}
...
void save_native_framebuffers(QEMUFile *f, GHashTable* resource_list) {
    ...
    LOGD("saving framebuffer num %d", g_hash_table_size(resource_list));
    ...
    while (g_hash_table_iter_next(&iter, &key, &value)) {
        ...
        LOGD("saving framebuffer ID: %d", framebuffer->framebufferId);
...
void restore_single_framebuffer(Express_Native_Framebuffer *framebuffer) {
    ...
    for(int i = 0; i < 16; i++) {
        if(framebuffer->attachment_target[i] != 0) {
            ...
            LOGD("loading framebuffer attachment %d old %d new %d", i, texture_id, attachment_type);
        }    
    }
    if(framebuffer->renderbuffer_attachment[0] != 0) {
        ...
        LOGD("loading framebuffer attachment depth old %d new %d", framebuffer->renderbuffer_attachment[0], GL_DEPTH_ATTACHMENT);
    }
    if(framebuffer->renderbuffer_attachment[1] != 0) {
        ...
        LOGD("loading framebuffer attachment GL_STENCIL_ATTACHMENT old %d type %d", framebuffer->renderbuffer_attachment[1], GL_DEPTH_ATTACHMENT);
        GLenum err = glGetError();
        if (err != GL_NO_ERROR) {
            LOGE("glFramebufferTexture2D error: %d", err);
            // 处理错误
        }
        GLint fbo_texture_id;
        glGetFramebufferAttachmentParameteriv(GL_FRAMEBUFFER, GL_STENCIL_ATTACHMENT, GL_FRAMEBUFFER_ATTACHMENT_OBJECT_NAME, &fbo_texture_id);
        LOGD("now framebuffer attachment GL_STENCIL_ATTACHMENT old %d new %d", framebuffer->renderbuffer_attachment[1], fbo_texture_id);
    }
    ...
    // LOGD("loading framebuffer ID: old %d new %d", framebuffer_id, new_framebuffer_id);
    ...
    //         LOGD("loading framebuffer attachment %d old %d new %d", j, attachment_object, new_attachment_object);
    //     } else {
    //         break;
    //     }
    // }
    ...
    if (glerror != GL_FRAMEBUFFER_COMPLETE) {
        LOGE("error! framebuffer not complete! status %x gl error %x fbo %d", glerror, glGetError(), framebuffer_id);
        ...
        // if (isValid) {
        //     LOGD("Texture is valid");
        // } else {
        //     LOGD("Texture is invalid");
        // }
    ...
    // LOGD("loading framebuffer all info %d %d %d", new_framebuffer_id, depth_attachment, stencil_attachment);
...
void load_native_framebuffers(QEMUFile *f, GHashTable* resource_list) {
    ...
    LOGD("get framebuffer num %d", framebuffer_num);
    for (int i = 0; i < framebuffer_num; i++) {
        ...
        for(int i = 0; i < 16; i++) {
            ...
            LOGD("loading framebuffer attachment %d %d", i, new_framebuffer->attachment_target[i]);
        }
        for(int i = 0; i < 19; i++) {
            new_framebuffer->renderbuffer_attachment[i] = get_host_id_map(RESOURCE_TYPE_RENDERBUFFER, qemu_get_be32(f));
            LOGD("loading framebuffer renderbuffer attachment %d %d", i, new_framebuffer->renderbuffer_attachment[i]);
        }
        // new_framebuffer->attachment_target = type;
        uint64_t texture_id = qemu_get_be64(f);
        new_framebuffer->texture_id = get_host_id_map(RESOURCE_TYPE_TEXTURE, texture_id);
        LOGD("in load native framebuffers loading framebuffer ID: %d, Texture ID: %d %d size %d", new_framebuffer->framebufferId, texture_id, new_framebuffer->texture_id, g_hash_table_size(resource_list));
...
void save_single_sampler(QEMUFile *f, GLint sampler_id) {
    LOGD("saving sampler ID: %d", sampler_id);
...
void save_native_samplers(QEMUFile *f) {
    ...
    LOGD("saving sampler num %d", g_hash_table_size(resource_list));
...
void save_single_renderbuffer(QEMUFile *f, GLint rb_id) {
    ...
    LOGD("saving renderbuffer ID: %d width %d height %d format %d samples %d", rb_id, width, height, format, samples);
}
...
void save_native_renderbuffers(QEMUFile *f) {
    ...
    qemu_put_be32(f, g_hash_table_size(resource_list)); //first save how many samplers
    LOGD("saving renderbuffer num %d", g_hash_table_size(resource_list));
...
void save_single_program_pipeline(QEMUFile *f, GLint pp_id) {
    LOGD("saving program_pipeline ID: %d", pp_id);
...
void load_single_program_pipeline(QEMUFile *f, GLint old_pp_id) {
    ...
    LOGD("loading program_pipeline ID: old %d new %d", old_pp_id, pp_id);
}
...
void save_native_program_pipelines(QEMUFile *f) {
    ...
    qemu_put_be32(f, g_hash_table_size(resource_list)); //first save how many samplers
    LOGD("saving program_pipeline num %d", g_hash_table_size(resource_list));
...
void load_single_sampler_tmp(QEMUFile *f, GLint old_sampler_id) {
    ...
    change_host_id_map(RESOURCE_TYPE_SAMPLER, old_sampler_id, old_sampler_id);
    LOGD("loading sampler ID: old %d new %d", old_sampler_id, sampler_id);
}
...
void load_native_samplers_tmp(QEMUFile *f) {
    int sampler_num = qemu_get_be32(f);
    for(int i = 0; i < sampler_num ; i++) {
        GLint old_sampler_id = qemu_get_be32(f);
        load_single_sampler_tmp(f, old_sampler_id);
    }
}
...
void load_single_renderbuffer(QEMUFile *f, GLint old_rb_id) {
    ...
    GLenum err = glGetError();
    if (err != GL_NO_ERROR) {
        LOGE("glRenderbufferStorage error: %d", err);
        // 处理错误
    }

    change_host_id_map(RESOURCE_TYPE_RENDERBUFFER, old_rb_id, rb_id);
    LOGD("loading renderbuffer ID: old %d new %d width %d height %d format %x samples %d", old_rb_id, rb_id, width, height, format, samples);
}

void load_single_renderbuffer_tmp(QEMUFile *f, GLint old_rb_id) {
    GLint rb_id;
    glGenRenderbuffers(1, (GLuint*)&rb_id);
    ...
    change_host_id_map(RESOURCE_TYPE_RENDERBUFFER, old_rb_id, old_rb_id);
    LOGD("loading renderbuffer ID: old %d new %d width %d height %d format %x samples %d", old_rb_id, rb_id, width, height, format, samples);
}
...
void load_native_renderbuffers_tmp(QEMUFile *f) {
    int rb_num = qemu_get_be32(f);
    for(int i = 0; i < rb_num ; i++) {
        GLint old_rb_id = qemu_get_be32(f);
        load_single_renderbuffer_tmp(f, old_rb_id);
    }
}
...
void save_virtqueue_element(QEMUFile *f, VirtQueueElement *elem) {
    ...
    // LOGD("in address is %llu out address is %llu in num %d out num %d", (uint64_t)in_address, (uint64_t)out_address, elem->in_num, elem->out_num);

    // LOGD("saving virtqueue element of in_address %llu out_address %llu in addr %llu out addr %llu", (uint64_t)in_real_guest_mem, (uint64_t)out_real_guest_mem, (uint64_t)elem->in_addr, (uint64_t)elem->out_addr);
...
void load_virtqueue_element(QEMUFile *f, VirtQueueElement *elem) {
    ...
    // LOGD("finish loading virtqueue element of in_address %p out_address %p", in_hva, out_hva);
}

void save_teleport_express_queue_elem(QEMUFile *f, Teleport_Express_Queue_Elem *elem) {
    LOGD("in save_teleport_express_queue_elem");
...
void load_teleport_express_queue_elem(QEMUFile *f, Teleport_Express_Queue_Elem *elem) {
    LOGD("in load_teleport_express_queue_elem");
...
void save_teleport_express_call(QEMUFile *f, Teleport_Express_Call *call) {
    ...
    if(call->vq == startup_in_data_queue) {
        qemu_put_be32(f, 1);
        LOGD("call type is in data queue");
...
Teleport_Express_Call* load_teleport_express_call(QEMUFile *f) {
    ...
    if(call_type == 1) {
        ...
        LOGD("call type is out data queue");
    } else {
...
void load_program_uniform_and_attrib_info(QEMUFile* f, GLuint program) {
    ...
    // LOGD("get uniform num %d attrib num %d", uniform_num, name_len);
    ...
    //     LOGD("in load program set uniform index %d name %s", location, name_buf);
    ...
    for(int i = 0; i < attrib_num; i++) {
        ...
        LOGD("in load program set attrib index %d name %s", location, name_buf);
...
void load_native_programs(QEMUFile *f){  //ztodo:应该先重新创建program、更新id！
    ...
    LOGD("program (%d -> %d) has shader num %d", program_id, new_program_id, shader_num);
        ...
        for(int j = 0; j < shader_num; j++) {
            ...
            LOGD("link program of %d shader %d", new_program_id, shader_attached_order[j]);
        ...
        for (int i = 0; i < uniform_num; i++) {
            ...
            LOGD("going to recover uniform value %d %d %d %d %d %s", new_program_id, location, old_loc, old_type, bufsize, name_buf);
            switch (old_type) {
                case GL_FLOAT:
                    {
                        GLfloat value;
                ...
                case GL_INT:
                    {
                        ...
                        LOGD("before loading get program uniform %d name %s index %d size %d value %d", program_id, name_buf, location, strlen(name_buf), value);
                    }
                    break;
                ...
                case GL_FLOAT_VEC4:
                    {
                        ...
                        LOGD("GL_FLOAT_VEC4 loading get program uniform %d name %s index %d size %d value %f %f %f %f", program_id, name_buf, location, strlen(name_buf), value[0], value[1], value[2], value[3]);
                ...
                case GL_SAMPLER_2D_SHADOW:
                    {
                        GLint value = (GLint)qemu_get_be64(f);
                        glUniform1i(location, value);
                        LOGD("samplers before loading get program uniform %d name %s index %d size %d value %d", program_id, name_buf, location, strlen(name_buf), value);
                    }
                    break;

                default:
                    LOGD("uniform type %x not supported", old_type);
                    break;
            }


            LOGD("loading get program uniform %d name %s index %d size %d type %x", program_id, name_buf, location, strlen(name_buf), old_type);
        ...
        LOGD("in load native programs loading program of %d %d", program_id, new_program_id);
    }
    ...
    for (int i = 0; i < external_program_num; i++) {
        ...
        LOGD("loading external program of id %d old %d", new_program_id, external_program);
    }
...
void save_program_uniform_and_attrib_info(QEMUFile* f, GLuint program) {
    ...
    for (int i = 0; i < attrib_num; i++){
        ...
        LOGD("saving get program attrib %d name %s index %d size %d", program, name_buf, location, strlen(name_buf));
    ...
    for (int i = 0; i < uniform_num; i++) {
        ...
        LOGD("saving get program uniform %d name %s index %d size %d", program, name_buf, location, strlen(name_buf));
            ...
            case GL_FLOAT_VEC4:
                {
                    ...
                    for(int i = 0; i < 4; i++) {
                        qemu_put_be32(f, *(uint32_t*)&value[i]);
                        LOGD("saving and get program uniform %d name %s index %d size %d value %f", program, name_buf, location, strlen(name_buf), value[i]);
                    }
                }
                break;
...
void save_native_programs(QEMUFile *f) {
    ...
    //     // LOGD("saving program ID:%lld %d, shader: %d",linked_program, (linked_program >> 32) & ((1 << 32) - 1), linked_program & ((1 << 32) - 1));
...
void update_native_shader(GLint shader_id, GLenum shader_type, GLboolean deleted_status, GLboolean compile_status, GLint source_length, const char* shader_source) {
    ...
    LOGD("created shader! host shader id %d type %d all %d", new_shader_id, shader_type, g_hash_table_size(resource_list));
    ATOMIC_UNLOCK(g_resource_locker[RESOURCE_TYPE_SHADER]);
}
...
void saveTextureAsPPM(const char *baseFilename, int textureId, int width, int height, unsigned char *pixels) {
    ...
    fclose(file);
    LOGD("Saved texture to %s\n", filename);

    LOGD("First pixels in RGBA format:");
    for (int i = 0; i < width * height; i++) {
        LOGD("Pixel %d: R=%d, G=%d, B=%d, A=%d\n",
               i, 
               pixels[i * 4 + 0], //R
               pixels[i * 4 + 1], //G
               pixels[i * 4 + 2], //B
...
void save_single_texture(QEMUFile *f, GLint texture_id, GLenum texture_type) {
    ...
    LOGD("get pixel size %d", pixel_size);
    ...
    LOGD("in saving texture of id %d target %d height %d width %d depth %d format %x pixels %d", texture_id, state->target, state->height, state->width, state->depth, state->internalFormat, size);
    ...
    //     LOGD("Memory is all zeros");
    // } else {
    //     // 说明 state->pixels 的内容不是全为 0
    //     LOGD("Memory is not all zeros");
...
void save_native_textures(QEMUFile *f){
    ...
    qemu_put_be32(f, g_hash_table_size(resource_list));
    LOGD("saving texture num %d", g_hash_table_size(resource_list));
...
void update_native_texture(Express_Native_Texture* texture_data){
    ...
    glGetIntegerv(GL_TEXTURE_BINDING_2D, &currentTexture); 
    LOGD("before native resource textures info current texture %d target %d", currentTexture, texture_data->target);
    ...
    if (texture_data->target == GL_TEXTURE_2D || texture_data->target == GL_TEXTURE_EXTERNAL_OES) {
        ...
        if (glerror != GL_NO_ERROR) {
            ...
            LOGD("success! loading texture glTexImage2D success! with id %d format %d internal %d %d %d", new_texture_id, format, texture_data->internalFormat, texture_data->target, texture_data->pixels);
        }
...
void load_native_textures_tmp(QEMUFile *f){
    ...
    LOGD("in load native textures! num %d", texture_num);
    ...
    for (int i = 0; i < texture_num; i++) {
        ...
        qemu_get_buffer(f, shader_source, source_length);
        LOGD("loading texture of id %d type %d", native_texture->textureId, native_texture->target);
...
//             LOGD("change texture id %d", native_texture->textureId);
...
int save_single_render_thread_context(QEMUFile *f, Render_Thread_Context *thread_context) {
    LOGD("in save single render thread context!");
    ...
    if (thread_context->process_context) {
        ...
        if(process != NULL) {
            ...
            LOGD("get process %lld from map %d", (thread_context->context).process_id, g_hash_table_size(process->surface_map));
        } else {
    ...
    if (thread_context->render_double_buffer_read) {
        ...
        if(window_buffer != NULL) {
            // LOGD("save render_double_buffer_read is not null!");
            LOGD("save render_double_buffer_read is not null! %lld %lld", (uint64_t)(thread_context->render_double_buffer_read)->guest_surface, GUINT_TO_POINTER((thread_context->render_double_buffer_read)->guest_surface));
        ...
        qemu_put_be64(f, 0);
        LOGD("render_double_buffer_read is null!");
    }
    if (thread_context->render_double_buffer_draw) {
        ...
        if(window_buffer != NULL) {
            LOGD("save render_double_buffer_draw is not null! %lld", (uint64_t)(thread_context->render_double_buffer_draw)->guest_surface);
        ...
        qemu_put_be64(f, 0);
        LOGD("render_double_buffer_draw is null!");
    }
...
Render_Thread_Context* load_single_render_thread_context(QEMUFile *f) {
    LOGD("in load single render thread context!");
    ...
    if(has_process) {
        ...
        LOGD("has process and get %lld %d %d", (uint64_t)(thread_context->context).process_id, thread_context->process_context->thread_cnt, g_hash_table_size(thread_context->process_context->surface_map));
    } else {
    ...
    if (has_read_window) {
        ...
        if(window_buffer != NULL) {
            LOGD("successfully load render buffer read of %lld", has_read_window);
    ...
    if (has_write_window) {
        ...
        if(window_buffer != NULL) {
            ...
            LOGD("successfully load render buffer write of %lld %lld", has_write_window, GUINT_TO_POINTER(has_write_window));
    ...
    if (has_opengl_context) {
        ...
        LOGD("has opengl context is null");
...
int save_thread_context(QEMUFile *f, Thread_Context *context) {
    ...
    LOGD("in save thread context with device id %lld thread id %lld process_id %lld unique_id %lld", context->device_id, context->thread_id, context->process_id, context->unique_id);
...
Render_Thread_Context* load_thread_context(QEMUFile *f) {
    ...
    LOGD("in load_thread_context with device id %lld thread id %lld process %lld unique %lld", device_id, thread_id, process_id, unique_id);
...
void save_process_context(QEMUFile *f, Process_Context *process_context) {
    LOGD("in save_process_context!");
    GHashTableIter iter;
    gpointer key, value;
    guint gbuffer_count = g_hash_table_size(process_context->gbuffer_map);
    LOGD("in save_process_context with gbuffer count %d", gbuffer_count);
    ...
    while (g_hash_table_iter_next(&iter, &key, &value)) {
        ...
        LOGD("save process context gbuffer id %lld", gbuffer_id);
        save_hardware_buffer(f, gbuffer); // host这边或许需要怎么重新处理一下
    }


    guint surface_count = g_hash_table_size(process_context->surface_map);
    LOGD("in save_process_context with surface_count count %d", surface_count);
    ...
    while (g_hash_table_iter_next(&iter, &key, &value)) {
        ...
        // LOGD("saving window buffer %lld %lld", (uint64_t)window_buffer->gbuffer->gbuffer_id, guest_surface_id);
    ...
    LOGD("in save_process_context with context_count count %d", context_count);
    ...
    LOGD("in save_process_context with thread count %d", process_context->thread_cnt);
...
void load_process_context(QEMUFile *f, Process_Context *process_context) {
    ...
    LOGD("in load_process_context with gbuffer count %d", gbuffer_count);

    for (guint i = 0; i < gbuffer_count; i++) {
        uint64_t gbuffer_id = qemu_get_be64(f);
        LOGD("load process context gbuffer id %lld", gbuffer_id);
    ...
    LOGD("in load_process_context with surface_count count %d", surface_count);

    for (guint i = 0; i < surface_count; i++) {
        uint64_t guest_surface_id = qemu_get_be64(f);
        LOGD("in load process context with id %lld %lld", guest_surface_id, GUINT_TO_POINTER(guest_surface_id));
    ...
    LOGD("process has surface map size %d", g_hash_table_size(process_context->surface_map));

    guint context_count = qemu_get_be32(f);
    LOGD("in load_process_context with context_count count %d", context_count);
    ...
    LOGD("in load_process_context with thread count %d", process_context->thread_cnt);
...
void save_window_buffer(QEMUFile *f, Window_Buffer *buffer, GHashTable* gbuffer_map) {
    ...
    // }
    // LOGD("in save_window_buffer! %d", sizeof(buffer->window_hints.hints));
...
//不确定这个guest_surface是这么存的啊，先这样吧 
    ...
    LOGD("in save_window_buffer! %lld %lld %d %d %d %d %d", buffer->guest_surface, buffer->gbuffer_id, buffer->width, buffer->height, buffer->is_current, buffer->need_destroy, buffer->swap_interval);
    ...
    if(gbuffer_id == 0){
        ...
        LOGD("save gbuffer from surface!");
        ...
        if(gbuffer == NULL) {
            ...
            LOGD("save gbuffer from global map of id %lld", gbuffer_id);
        } 
...
int load_window_buffer(QEMUFile *f, Window_Buffer *buffer, GHashTable* gbuffer_map) {
    ...
    LOGD("in load_window_buffer! %d", sizeof(buffer->window_hints.hints));
    ...
    LOGD("in load_window_buffer! %lld %lld %d %d %d %d %d", buffer->guest_surface, buffer->gbuffer_id, buffer->width, buffer->height, buffer->is_current, buffer->need_destroy, buffer->swap_interval);
    for (int i = 0; i < 20; i++) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        //ztodo:它们的id没有更新！！！！
        LOGD("load window buffer fbo %d %d %d %d %d", i, buffer->data_fbo[i], buffer->sampler_fbo[i], buffer->connect_texture[i], old_texture);
    }
    ...
    if(target == 0) {
        ...
        LOGD("load gbuffer from global map of id %lld", gbuffer_id);
    } else {
        uint64_t gbuffer_id = qemu_get_be64(f);
...
#ifdef __APPLE__
    real_guest_mem = (void *)address;
#endif

        qemu_put_be64(f, (uint64_t)real_guest_mem);
    // LOGD("scatter data size is %d data %llx %llx", sizeof(scatter_data[i].iov_base), address, (uint64_t)real_guest_mem);

    // LOGD("saving scatter data %d %lld", (int)scatter_data[i].iov_base, (uint64_t)scatter_data[i].iov_base);
        // qemu_put_buffer(f, scatter_data[i].iov_base, scatter_data[i].iov_len);
    }
}
...
Scatter_Data* load_scatter_data(QEMUFile *f, int *count) {
    *count = qemu_get_be32(f);
    ...
    // LOGD("scatter data size is %d", sizeof(data));
    ...
    // LOGD("loading scatter data len %d", len);

        uint64_t address = qemu_get_be64(f); //存的是gpa
        LOGD("loading scatter data address %llu", address);
    ...
    // LOGD("loading scatter data %d %lld", (int)scatter_data[i].iov_base, (uint64_t)scatter_data[i].iov_base);
        ...
        LOGD("loading scatter data len %d %d %llu %llu", len, *count, (unsigned long long)hva, (unsigned long long)address);
    }
...
Guest_Mem* load_guest_mem(QEMUFile *f, int strategy) {
    ...
    if(strategy == 0) {
        ...
        LOGD("in 1 get guest mem pointer %lld size %d", (uint64_t)guest_mem, sizeof(Guest_Mem));
    } else {
        guest_mem = alloc_one_guest_mem();
    }
    // Guest_Mem *guest_mem = alloc_one_guest_mem();//g_malloc0(sizeof(Guest_Mem));
    
    LOGD("get guest mem pointer %lld size %d", (uint64_t)guest_mem, sizeof(Guest_Mem));
...
void save_hardware_buffer(QEMUFile *f, Hardware_Buffer *buffer) {
    ...
    LOGD("save hardware buffer texture id %llx %d %d", buffer->gbuffer_id, buffer->data_texture, buffer->data_fbo);
}
...
Hardware_Buffer* load_hardware_buffer(QEMUFile *f) {
    ...
    LOGD("load hardware buffer texture id %d %d", buffer->data_texture, new_texture);
    ...
    if (buffer->depth_internal_format != 0)
    {
        ...
        LOGD("in load gbuffer gen depth of id %d", buffer->rbo_depth);
    ...
    {
        LOGD("in load gbuffer gen stencil of id %d", buffer->rbo_stencil);
...
void save_attrib_point(QEMUFile *f, Attrib_Point *point) {
    ...
    for(int i = 0; i < MAX_VERTEX_ATTRIBS_NUM; i++) {
        qemu_put_be32(f, point->divisors[i]);
    }
}
...
Attrib_Point* load_attrib_point(QEMUFile *f) {
    ...
    for (int i = 0; i < MAX_VERTEX_ATTRIBS_NUM; i++) {
        ...
        LOGD("load attrib point vbo %d %d %d %d", point->buffer_object[i], point->buffer_loc[i], point->remain_buffer_len[i], point->buffer_len[i]);
    }
    ...
    for(int i = 0; i < MAX_VERTEX_ATTRIBS_NUM; i++) {
        point->divisors[i] = qemu_get_be32(f);
    }

    LOGD("load attrib point ebo %d %d %d %d", point->indices_buffer_object, point->indices_buffer_len, point->remain_indices_buffer_len, point->element_array_buffer);
    return point;
}
...
void save_buffer_status(QEMUFile *f, Buffer_Status *status) {
    ...
    LOGD("guest and host vao value %d %d", status->guest_vao, status->host_vao);
}
...
Buffer_Status* load_buffer_status(QEMUFile *f) {
    ...
    LOGD("loading buffer status vbo %d %d ebo %d %d", status->guest_array_buffer, status->host_array_buffer, status->guest_element_array_buffer, status->host_element_array_buffer);
    ...
    GLint old_guest_pixel_pack_buffer = qemu_get_be32(f);
    GLint old_host_pixel_pack_buffer = qemu_get_be32(f);
    status->guest_pixel_pack_buffer = get_host_id_map(RESOURCE_TYPE_BUFFER, old_guest_pixel_pack_buffer);
    status->host_pixel_pack_buffer = get_host_id_map(RESOURCE_TYPE_BUFFER, old_host_pixel_pack_buffer);
    ...
    LOGD("loading buffer status pixel pack %d %d pixel unpack %d %d", old_guest_pixel_pack_buffer, old_host_pixel_pack_buffer, status->guest_pixel_pack_buffer, status->host_pixel_pack_buffer);
...
void save_bound_buffer(QEMUFile *f, Bound_Buffer *buffer) {
    ...
    while (g_hash_table_iter_next(&iter, &key, &value)) {
        ...
        //     LOGD("in save attrib Attrib %d: Divisor = %d", i, divisor);
        // }
...
void save_resource_map_status(QEMUFile *f, Resource_Map_Status *status) {
    LOGD("save resource map status with map_size %d max size %d", status->map_size, status->gbuffer_map_max_size);
    ...
    for (unsigned int i = 0; i < status->gbuffer_map_max_size; i++) {
        if (status->gbuffer_id_map[i] != 0 && get_gbuffer_from_global_map(status->gbuffer_id_map[i]) != NULL) {
            ...
            LOGD("save resource map status with gbuffer id %lld id %d", status->gbuffer_id_map[i], i);
...
Resource_Map_Status* load_resource_map_status_without_share(QEMUFile *f, int resource_type) { //不可共享的资源要在别的地方映射（在线程中创建并映射）
    ...
    for (unsigned int i = 0; i < status->map_size; i++) {
        ...
        LOGD("finish not share loading guest id %d host %d %d %d %d", i, resource_type, status->resource_id_map[i], status->resource_is_init[i], g_hash_table_size(g_resource_ids_map[resource_type]));
    }
...
void save_resource_context(QEMUFile *f, Resource_Context *context) {
    ...
    LOGD("finish saving texture resource!");
...
Resource_Context* load_resource_context(QEMUFile *f) {
    ...
    LOGD("finish loading texture resource!");
...
void save_texture_binding_status(QEMUFile *f, Texture_Binding_Status *status) {
    ...
    LOGD("in save texture binding status with max unit %d %d %d", status->now_max_texture_unit, sizeof(status->guest_current_texture_2D)/sizeof(int), status->texture_unit_num);
...
Texture_Binding_Status* load_texture_binding_status(QEMUFile *f) { //ztodo:这些都得改
    ...
    LOGD("in load texture binding status with max unit %d %d", tot_num, status->now_max_texture_unit);
...
void save_opengl_context(QEMUFile *f, Opengl_Context *context) {
    ...
    LOGD("in save opengl context of %llx %llx", context, context->window);

    //不确定是否需要深拷贝，先浅拷贝了 update:需要额外处理，先注释了
    // qemu_put_be64(f, context->window);
    // LOGD("the native context saving now is %llx", (int64_t)context->window);
    ...
    // LOGD("in save opengl context with fbo0 %d %d", context->draw_fbo0, context->read_fbo0);
    // ATOMIC_UNLOCK(g_resource_locker[RESOURCE_TYPE_FRAMEBUFFER]);
    ...
    LOGD("in save opengl context with fbo0 %d %d %d", context->draw_fbo0, context->read_fbo0, context->vao0);
    ...
    LOGD("in save opengl context with num_entries %d", num_entries);
    ...
    for(int i = 0; i < 4 ; i++) {
        qemu_put_be32(f, context->gl_scissor_value[i]);
    }
}
...
void restore_opengl_context_textures(Opengl_Context *context) {
    ...
    LOGD("The currently bound GL_TEXTURE_2D ID is: %d should bind is %d", textureId, status->guest_current_texture_2D[current_active_texture]);
    ...
    LOGD("The currently bound GL_TEXTURE_2D_MULTISAMPLE ID is:%d %d", status->guest_current_texture_2D_multisample[current_active_texture], textureId);
    ...
    LOGD("The currently bound GL_TEXTURE_2D_MULTISAMPLE_ARRAY ID is: %d %d", textureId, status->guest_current_texture_2D_multisample_array[current_active_texture]);
    glerror = glGetError();
    if (glerror != GL_NO_ERROR) {
    ...
    LOGD("The currently bound GL_TEXTURE_3D ID is: %d", textureId);
...
int load_opengl_context(QEMUFile *f, Opengl_Context *context) {
    ...
    LOGD("in load opengl context of %llx %llx", context, context->window);
    ...
    // LOGD("in load opengl context with draw_fbo0 %d %d", context->draw_fbo0, new_draw_fbo);
    ...
    LOGD("in load opengl context with fbo0 %d %d %d", context->current_read_fbo, context->current_write_fbo, context->current_program);
    ...
    LOGD("in load opengl context with num_entries %d", num_entries);
    for (guint i = 0; i < num_entries; i++) {
        ...
        LOGD("loading opengl buffer map of target %x old id %d new id %d", target, buffer_id, new_buffer_id);
    ...
    for(int i = 0; i < 4 ; i++) {
        context->gl_scissor_value[i] = qemu_get_be32(f);
    }
