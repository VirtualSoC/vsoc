...
#include "hw/vsoc/mem/express_mem.h"
...
void d_glBindEGLImage(void *t_context, GLenum target, uint64_t image, GLuint texture, GLuint share_texture, EGLContext guest_share_ctx)
{
    ...
    if (gbuffer->usage_type != GBUFFER_TYPE_TEXTURE) //基本都会进这里
    {
        ...
        LOGD("glBindEGLImage gbuffer %p id %llx is_writing %d sync %d texture %d %d", gbuffer, gbuffer_id, gbuffer->is_writing, gbuffer->data_sync, texture, gbuffer->data_texture);
    ...
    if (origin_texture > 0)
    {
        ...
        if (glerror != GL_NO_ERROR) {
            LOGD("glBindEGLImage delete texture %d failed! glerror %x", origin_texture, glerror);
        }
