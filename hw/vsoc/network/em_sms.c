...
#include "hw/vsoc/network/em_gsm.h"
...
sms_receiver_add_submit_pdu( SmsReceiver  rec, SmsPDU       submit_pdu )
{
    ...
    if ( smspdu_get_receiver_address( submit_pdu, from ) < 0 ) {
        LOGD( "%s: could not extract receiver address\n", __FUNCTION__ );
        return -1;
    }

    ref = smspdu_get_ref( submit_pdu );
    if (ref < 0) {
        LOGD( "%s: could not extract message reference from pdu\n", __FUNCTION__ );
        return -1;
    }
    max = smspdu_get_max_index( submit_pdu );
    if (max < 0) {
        LOGD( "%s: invalid max fragment value: %d should be >= 1\n",
           __FUNCTION__, max );
        return -1;
    }
    ...
    if (frag == NULL) {
        ...
        if (frag == NULL) {
            LOGD("%s: not enough memory to allocate new fragment\n", __FUNCTION__ );
            return -1;
        }
#ifdef STD_DEBUG_LOG
            char  tmp[32];
            ...
            LOGD("%s: created SMS index %d, from %.*s, ref %d, max %d\n", __FUNCTION__,
    ...
    if (cur < 0) {
        LOGD("%s: SMS fragment index is too small: %d should be >= 1\n", __FUNCTION__, cur+1 );
        return -1;
    }
    if (cur >= max) {
        LOGD("%s: SMS fragment index is too large (%d >= %d)\n", __FUNCTION__, cur, max);
        return -1;
    }
    if ( frag->pdus[cur] != NULL ) {
        LOGD("%s: receiving duplicate SMS fragment for %d/%d, ref=%d, discarding old one\n",
    ...
    if (frag->count >= frag->max) {
        ...
        LOGD( "%s: SMS index %d, received all %d fragments\n", __FUNCTION__, frag->index, frag->count );
        return frag->index;
    }
    else {
        /* still waiting for more */
        LOGD( "%s: SMS index %d, received %d/%d, waiting for %d more\n", __FUNCTION__,
...
sms_receiver_get_text_message( SmsReceiver  rec, int  index, bytes_t  utf8, int  utf8len )
{
    ...
    if (frag == NULL) {
        LOGD( "%s: invalid SMS index %d\n", __FUNCTION__, index );
        return -1;
    }
    if (frag->count != frag->max) {
        LOGD( "%s: SMS index %d still needs %d fragments\n", __FUNCTION__,
...
sms_receiver_create_deliver( SmsReceiver  rec, int  index, const SmsAddressRec*  from )
{
    ...
    if (utf8 == NULL) {
        LOGD( "%s: not enough memory to allocate %d bytes\n",
