...
static void container_hmp_handler(Monitor *mon, int argc, const char **argv) {
    if (argc == 1 && strcmp(argv[0], "new") == 0) {
        ...
        char *payload = g_malloc(cmd->len + 1);
        memcpy(payload, cmd->str, cmd->len);
        payload[cmd->len] = '\0';
        char *resp = g_malloc0(VSOC_IPC_MAX_PAYLOAD);
        uint32_t resp_len = VSOC_IPC_MAX_PAYLOAD;
        int rc = vsoc_ipc_request(worker->ctx, VSOC_IPC_TYPE_HMP_COMMAND,
