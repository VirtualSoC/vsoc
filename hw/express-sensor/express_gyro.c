...
void sync_express_gyro_status(void)
{
    ...
    if (!static_gyro_context.need_sync || !gyro_irq_enable)
    {
        return;
    }
    ...
    printf("gyro irq send ok\n");

    send_express_device_irq(static_gyro_context.irq_call, 0, sizeof(Express_Gyro_Data));
