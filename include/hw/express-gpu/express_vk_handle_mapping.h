...
typedef enum ExpressVkObjectType {
    EXPRESS_VK_OBJECT_TYPE_UNKNOWN = 0,
    ...
    // ---- 以下为自定义扩展，不存在于官方枚举 ----
    EXPRESS_VK_OBJECT_TYPE_SURFACE,       // 自定义内部 surface 对象（如非KHR实现）
    EXPRESS_VK_OBJECT_TYPE_NATIVE_WINDOW, // guest 或 host 的 native window 封装
